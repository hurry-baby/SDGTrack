...
from SDGTrack.lib.tracker.SDGTrack import JDETracker#
from SDGTrack.lib.mot_online.log import logger
from SDGTrack.lib.mot_online.timer import Timer
from SDGTrack.lib.mot_online.evaluation import Evaluator

from SDGTrack.lib.mot_online.utils import mkdir_if_missing
from SDGTrack.lib.dataset.SDGTrack import LoadImages_jde
...
#自制画追踪结果
def draw_tracking_result(image, tlwhs, ids, frame_id, fps):
    ...
    for tlwh, track_id in zip(tlwhs, ids):
        ...
        cv2.rectangle(im, (x, y), (x + w, y + h), (0, 255, 0), 2)  # 画框
        cv2.putText(im, str(track_id), (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  # 显示ID

    # 在图像上显示帧信息
    cv2.putText(im, f"Frame: {frame_id}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    cv2.putText(im, f"FPS: {fps:.2f}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
def eval_seq(opt, dataloader, data_type, result_filename,seq,save_dir=None, show_image=True, frame_rate=30):
    if save_dir:
        mkdir_if_missing(save_dir)#创建保存文件夹
    ...
    for path, img, img0,frame_id_start in dataloader:
        ...
        online_targets = tracker.update(blob, img0, seq,save_dir)# 检测 跟踪
        online_tlwhs = []
        online_ids = []
        for t in online_targets:
            tlwh = t.tlwh
            tid = t.track_id
            # vertical = tlwh[2] / tlwh[3] > 0.6  #横纵比值限制。。。。。。。。。。。。啊啊啊啊啊啊啊啊啊啊啊啊啊啊啊
            ...
            # if tlwh[2] * tlwh[3] > opt.min_box_area and not vertical:#还有这。。。。。。。。。。。
        ...
        if show_image or save_dir is not None:#原来是vis.plot_tracking
            online_im = draw_tracking_result(img0, online_tlwhs, online_ids, frame_id=frame_id,
...
def main(opt, data_root='/data/MOT16/train', det_root=None, seqs=('MOT16-05',), exp_name='demo',
    ...
    result_id = []
    for seq in seqs:
        output_dir = os.path.join('../vis')#原来../vis
        ...
        nf, ta, tc, detection_num,id_num,frame_id_start = eval_seq(opt, dataloader, data_type, result_filename,seq,
                              save_dir=output_dir, show_image=show_image, frame_rate=frame_rate) #主要 检测追踪
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_mot16', default=True, help='val mot16 or mot15')
    ...
    parser.add_argument('--input_video', type=str, default='G:/pythonchengxu/SDGTrack/dataset/MOT16/test/MOT Challenge - Visualize.mp4',
    ...
    parser.add_argument('--vis_state', type=int, default=1, help='default=0,1 for vision or heatmap and reid')

    parser.add_argument('--weights',type=str, default='../weights/best_mot_test.pt', help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='../experiments/model_set/SDGTrack.yaml', help='model.yaml path')
    ...
    parser.add_argument('--data_dir', type=str, default='G:/pythonchengxu/SOTS-MOT/SDGTrack/dataset')
