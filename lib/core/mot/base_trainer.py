...
class MotLoss(torch.nn.Module):
    ...
    def forward(self,output, targets, model):
        ...
        self.s_id = nn.Parameter(0.02*torch.ones(1)).to(device) # 调整ID loss的权重 初始化为0.02
        ...
        # Focal loss
        g = h['fl_gamma']  # focal loss gamma  旨在解决类别不平衡的问题
        if g > 0:
            BCEcls, BCEobj = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g)
        ...
        np = len(p)  # number of outputs
        balance = [4.0, 1.0, 0.4] if np == 3 else [4.0, 1.0, 0.4, 0.1]  # P3-5 or P3-6
        for i, pi in enumerate(p):  # layer index, layer predictions   i: 当前处理的输出层的索引  pi: 当前输出层的预测
            b, a, gj, gi = indices[i]  # image, anchor, gridy, gridx
            tobj = torch.zeros_like(pi[..., 0], device=device)  # target obj  目标对象，一个与当前输出层形状相同的零 张 量，用于表示目标在当前层的存在
            ...
            if n:
                ...
                pxy = ps[:, :2].sigmoid() * 2. - 0.5
                pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[i]
                pbox = torch.cat((pxy, pwh), 1).to(device)  # predicted box 得到预测框
                giou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # giou(prediction, target)
                lbox += (1.0 - giou).mean()  # giou loss 框损失（用的glou损失进行计算）

                # Objectness
                tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(tobj.dtype)  # giou ratio

                # Classification 如果是多个类别计算分类损失
                if model.nc > 1:  # cls loss (only if multiple classes)
            ...
            lobj += BCEobj(pi[..., 4], tobj) * balance[i]  # obj loss 目标损失
