...
from models.mot.SDGTrack import Model
...
class JDETracker(object):
    ...
    def update(self, im_blob, img0, seq_num, save_dir):
        ...
        # dists1 = matching.fuse_motion(self.kalman_filter, dists, strack_pool, detections)
        dists = matching.iou_distance(strack_pool, detections)
        # dists = 0.01*dists1 + dists2
        ...
        # Predict the current location with KF
        # STrack.multi_predict(u_track)
        # dists = matching.embedding_distance(strack_pool, detections)
        # dists1 = matching.fuse_motion(self.kalman_filter, dists, strack_pool, detections)
        # dists_emb = matching.embedding_distance(strack_pool, detections)
        # dists1 = matching.fuse_motion(self.kalman_filter, dists_emb, strack_pool,detections)

        r_tracked_stracks = [strack_pool[i] for i in u_track if strack_pool[i].state == TrackState.Tracked]
        dists = matching.iou_distance(r_tracked_stracks, detections)
        # dists2 = matching.iou_distance(r_tracked_stracks, detections)
        # dists_id = matching.embedding_distance(r_tracked_stracks, detections)
        # dists_id = matching.fuse_motion(self.kalman_filter, dists_id, r_tracked_stracks, detections)
        # dists =0.9*dists1+0.1*dists_id
        ...
        # r_tracked_stracks = [strack_pool[i] for i in u_track if strack_pool[i].state == TrackState.Tracked]
        #
        # dists1 = matching.iou_distance(r_tracked_stracks, detections)
        # # dists2 = matching.iou_distance(r_tracked_stracks, detections)
        # dists_id = matching.embedding_distance(r_tracked_stracks, detections)
        # dists = matching.fuse_motion(self.kalman_filter, dists_id, r_tracked_stracks, detections)
        # # dists =0.9*dists1+0.1*dists_id
        ...
        dists1 = matching.iou_distance(second_tracked_stracks, detections_second)

        dists_id = matching.embedding_distance(second_tracked_stracks, detections_second)
        dists_id = matching.fuse_motion(self.kalman_filter, dists_id, second_tracked_stracks, detections_second)
        dists =0.2*dists1+0.2*dists_id

        matches, u_track, u_detection_second = matching.linear_assignment(dists, thresh=0.3)
        for itracked, idet in matches:
