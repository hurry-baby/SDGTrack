...
```
conda create -n SDGTrack python=3.8
source activate SDGTrack
cd SDGTrack/lib/tutorial/SDGTrack/
...
2. Download testing data e.g. MOT-16 and put them in `$SDGTrack/dataset`. The dataset can be downloaded from their [official webpage](https://motchallenge.net/).


### Testing
In root path `$SDGTrack/tracking`,

#### For MOT-16
```
python test_SDGTrack.py --nms_thres 0.6
                       --conf_thres 0.5
                       --weights $SDGTack/weights/SDGTrack.pt
                       --data_dir $SDGTrack/dataset
                       --device 0
                       --test_mot16 True
                       --vis_state 0/1
```
...
python test_SDGTrack.py --nms_thres 0.6
                       --conf_thres 0.5
                       --weights $SDGTrack/weights/SDGTrack.pt
                       --data_dir $SDGTrack/dataset
                       --device 0
                       --test_mot17 True
                       --vis_state 0/1
...
1. Download the pretrained model which pretrain in COCO dataset [[Google Drive]](https://drive.google.com/file/d/1qJHNlEXPVirDVmWL7hHeU4-P9amWHJHR/view?usp=sharing)[[Baidu NetDisk(ba1g)]](https://pan.baidu.com/s/1S04i6-yxQ3QHtfUDDtd1Kw) to `$SDGTrack/weights`.

2. We provide several relevant datasets for training and evaluating the SDGTrack. 
...
### Training
1. Modify scripts
Set the dataset path in line2 of `$SDGTrack/lib/dataset/mot/cfg/*.json`.

2. run
```
python train_SDGTrack.py --batch_size 8 --device 0 --weights $SDGTrack/weights/yolov5l.pt --epochs 30 --data ../lib/dataset/mot/cfg/data_ch.json #train on all datasets
